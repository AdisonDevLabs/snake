...
// Helper to generate path: "Ice Cream Cone" -> "public/images/gifts/ice_cream_cone.webp"
...
const teamABoostGiftSelect = document.getElementById('teamABoostGift'); // New
const teamBBoostGiftSelect = document.getElementById('teamBBoostGift'); // New
...
// Disconnect Logic Variables
let disconnectInterval = null;
let disconnectTimeLeft = 60;
...
// ============================================
// 1. INITIALIZATION & CONFIGURATION LOGIC
// ============================================
...
function applyConfiguration() {
    ...
    // Button Icon
    btnIconA.src = AppConfig.teamA.giftImg; // Manual button uses normal gift
    ...
    btnLabelA.innerText = AppConfig.teamA.name;
    btnIconA.innerText = AppConfig.teamA.giftIcon;
    btnTeamA.style.backgroundColor = AppConfig.teamA.color;
    
    btnLabelB.innerText = AppConfig.teamB.name;
    btnIconB.innerText = AppConfig.teamB.giftIcon;
    btnTeamB.style.backgroundColor = AppConfig.teamB.color;
    ...
    // Combined with CSS 'lg:hidden', this ensures they only show on small screens if Manual is on
...
socket.on('tiktok_connected', (data) => {
    ...
    if (isGameRunning && isPaused) {
        ...
        statusText.innerHTML = "GAME RESUMED";
...
socket.on('disconnect', () => {
    ...
    if (isGameRunning) {
        ...
        disconnectTimeLeft = 60;
        statusText.innerHTML = `⚠️ CONNECTION LOST ⚠️<br>STOPPING IN ${disconnectTimeLeft}s`;
...
// UPDATE: spawnFood now accepts isBoost
...
function update() {
    // 1. Determine Velocity based on Mode
    if (AppConfig.gameMode === 'manual') {
    ...
    if (foodIdx !== -1) {
        ...
        const points = food.isBoost ? 500 : 50; // 500 for Boost, 50 for Normal
        const tone = food.isBoost ? 800 : 600;  // Higher pitch for boost
...
function draw() {
    ...
    foods.forEach(f => {
        ...
        if (f.type === 'teamA') {
            ...
            radius = (Math.min(w, h) / 2) * (f.isBoost ? 1.5 : 1.3); // Bigger if boost
